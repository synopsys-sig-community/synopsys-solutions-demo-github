...
const stock = async (productId) => {
    let endPoint = "/" + productId + "/stock";
        
    data = await inventory (endPoint, "stock");
    console.info("stock: " + data);
    return data;
};

const deliveryDate = async (productId) => {
    let endPoint = "/" + productId + "/delivery";
    
    data = await inventory (endPoint, "delivery");
    console.info("ship date: " + data);
    return data;

};


const inventory = async (endPoint, key) => {
    const config = getConfig();

    data = ""  

    await axios.get (config.inventoryAPI + endPoint)  
    .then ((response) => {
        if(response.status === 200) {
            console.info('Successfully called inventory API');
        }
    ...
    .catch((err) => {
        console.log('Error calling inventory API:', err);
    });
